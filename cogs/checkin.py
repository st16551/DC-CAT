...
class PersistentCheckinView(discord.ui.View):
    ...
    async def checkin_button_callback(
        ...
        cursor.execute(
            "SELECT last_date, streak FROM checkin_system WHERE discord_id = ?",
            (user_id,),
        )
        ...
        if isinstance(xp_result, dict) and xp_result.get("leveled_up"):
...
class CheckinCog(commands.Cog):
    ...
    @app_commands.command(
        name="建立簽到面板", description="[幹部專用] 發送永久有效的簽到卡片"
    )
    ...
    async def reset_checkin(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "🗑️ **【系統重製完成】** 所有人的簽到紀錄與天數已全數歸零！", ephemeral=True
        )
