...
async def execute_market_sync(bot=None):
  """將 API 抓取與盯盤掃描抽離成獨立函數，精準對應遊戲市場 API 的 unit_price 與名稱"""
  ...
  try:
      ...
      if not raw_items:
        for val in data.values():
          if isinstance(val, list):
            raw_items = val
            break
      if not raw_items and ("name" in data or "item_name" in data):
        raw_items = [data]
    ...
    for item in raw_items:
      ...
      # 💡 優先鎖定 unit_price，確保抓到的是「單價」而非總價
      price_raw = (
          item.get("unit_price")
  ...
  async def on_submit(self, interaction: discord.Interaction):
    ...
    if not rows:
      await interaction.followup.send(
          f'🔍 找不到與 `"{keyword}"` 相關的市場行情，請確認名稱或英文拼寫是否正確！',
          ephemeral=True,
      )
      return
